...
async function apiRequest(method, endpoint, body) {
  ...
  try {
    ...
    const message = error?.name === 'AbortError'
      ? `API request timeout after 15000ms: ${method} ${endpoint}`
      : `API request failed for ${method} ${endpoint}: ${error.message || String(error)}`;
...
class Dataset {
  ...
  async pushData(items) {
    ...
    for (let i = 0; i < arr.length; i += 500) {
      ...
      log('info', `[Dataset] Pushing chunk ${i / 500 + 1} with ${chunk.length} items`);
